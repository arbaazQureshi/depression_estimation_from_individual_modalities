...
if(path.exists('min_model.h5')):
	model = load_model(Tx, n_x, location='min_model.h5')
...
while(current_epoch_number < no_of_downward_epochs):
	...
	np.savetxt('learner_params.txt', np.array([min_loss_dev, prev_loss_dev, loss_dev, increase_count, current_epoch_number, total_epoch_count]), fmt='%.4f')
