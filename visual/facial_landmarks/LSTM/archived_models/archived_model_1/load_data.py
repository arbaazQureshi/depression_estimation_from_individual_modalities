...
def load_training_data():
	...
	for i in range(1,6):
		print(i, end='\r')
		location = '/home/syedcs15/depression_estimation/preprocessed_data/training_data/CLNF/preprocessed_visual_features/training_visual_features_batch_'+str(i)+'.npy'
		X.append(np.load(location))
	...
	for (score, gender) in zip(y, x_gender):

		y_.append(score)
		x_gender_.append(gender)

		if(i%batch_size == 0):
			Y.append(y_)
			X_gender.append(x_gender_)
	...
	if(len(train_set_ID_list)%batch_size != 0):
		Y.append(y_)
		X_gender.append(x_gender_)


	for i in range(len(X)):
		X_gender[i] = np.array(X_gender[i])
...
def load_development_data():
	...
	location = '/home/syedcs15/depression_estimation/preprocessed_data/development_data/CLNF/preprocessed_visual_features/development_visual_features.npy'
	X = np.load(location)
	...
	X_gender = labels['Gender'][dev_set_ID_list].values.tolist()
